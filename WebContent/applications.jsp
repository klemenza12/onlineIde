<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="edu.neu.cs5200.s3.onlineide.applications.*, java.util.*"%>
...
<body>
	...
	<%
	 ...
	 //Controller
		...
		if("create".equals(action) && (name != "") && (price != "")){
	   	...
	   	<table class="table">
			...
			%>	
				<tr>
					<td><%= ap.getName()  %></td>	
					<td><%= ap.getPrice() %></td>
					<td><a class="btn btn-danger" href="applications.jsp?action=remove&id=<%= ap.getId() %>" >Delete</a>
						<a class="btn btn-warning" href="applications.jsp?action=select&id=<%= ap.getId() %>" >Select</a>
					</td>	
				</tr>	
			<%
				}
